...
#### Rust Client
- New `commitdb` Rust crate at `clients/rust/` with blocking TCP/TLS support
- `ConnectOptions` builder for host, port, TLS, JWT, and timeout configuration
- Typed results: `QueryResult`, `CommitResult`, `ExecuteResult` enum
- `thiserror`-based `Error` enum with `From` impls for `io::Error`, `serde_json::Error`, `native_tls::Error`
- JWT authentication via `authenticate_jwt()`
- Auto-cleanup via `Drop` implementation
- Documentation at `docs/rust-client.md`
