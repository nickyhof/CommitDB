...
func setupBenchmarkDB(b *testing.B) *engine.Engine {
...
func BenchmarkSelectWithIn(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := e.Execute("SELECT * FROM bench.users WHERE city IN ('City1', 'City2', 'City3')")
		if err != nil {
			b.Fatalf("Execute error: %v", err)
		}
	}
...
func BenchmarkUpdate(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		id := (i % 1000) + 1
		_, err := e.Execute("UPDATE bench.users SET age = 99 WHERE id = " + strconv.Itoa(id))
		if err != nil {
...
func BenchmarkDistinct(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := e.Execute("SELECT DISTINCT city FROM bench.users")
		if err != nil {
			b.Fatalf("Execute error: %v", err)
		}
	}
...
func BenchmarkBulkInsert(b *testing.B) {
	e := setupBenchmarkDB(b)
	e.Execute("CREATE TABLE bench.bulk_test (id INT PRIMARY KEY, name STRING, value INT)")
	...
	for i := 0; i < b.N; i++ {
		...
		_, err := e.Execute("INSERT INTO bench.bulk_test (id, name, value) VALUES " + values)
		if err != nil {
...
func BenchmarkCopyIntoExport(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := e.Execute("COPY INTO '" + exportPath + "' FROM bench.users")
		if err != nil {
...
func BenchmarkCopyIntoImport(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		e.Execute("CREATE TABLE bench." + tableName + " (id INT PRIMARY KEY, name STRING, age INT, city STRING)")

		_, err := e.Execute("COPY INTO bench." + tableName + " FROM '" + exportPath + "'")
		if err != nil {
...
func BenchmarkJoin(b *testing.B) {
	...
	e.Execute("CREATE TABLE bench.orders (id INT PRIMARY KEY, user_id INT, amount INT)")
	for i := 0; i < 100; i++ {
		e.Execute(fmt.Sprintf("INSERT INTO bench.orders (id, user_id, amount) VALUES (%d, %d, %d)", i, i%50, (i+1)*10))
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := e.Execute("SELECT u.name, o.amount FROM bench.users u INNER JOIN bench.orders o ON u.id = o.user_id")
		if err != nil {
			b.Fatalf("Execute error: %v", err)
		}
	}
...
func BenchmarkStringFunctions(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := e.Execute("SELECT UPPER(name), LOWER(city), LENGTH(name) FROM bench.users")
		if err != nil {
			b.Fatalf("Execute error: %v", err)
		}
	}
...
func BenchmarkView(b *testing.B) {
	...
	e.Execute("CREATE VIEW bench.senior_users AS SELECT * FROM bench.users WHERE age > 40")
	...
	b.Run("ViewQuery", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, err := e.Execute("SELECT * FROM bench.senior_users")
			if err != nil {
				b.Fatalf("Execute error: %v", err)
			}
		}
...
func BenchmarkMaterializedView(b *testing.B) {
	...
	e.Execute("CREATE MATERIALIZED VIEW bench.city_stats AS SELECT city, COUNT(*), AVG(age) FROM bench.users GROUP BY city")
	...
	b.Run("MaterializedQuery", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, err := e.Execute("SELECT * FROM bench.city_stats")
			if err != nil {
				b.Fatalf("Execute error: %v", err)
			}
		}
	...
	b.Run("RefreshView", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, err := e.Execute("REFRESH VIEW bench.city_stats")
			if err != nil {
				b.Fatalf("Execute error: %v", err)
			}
		}
